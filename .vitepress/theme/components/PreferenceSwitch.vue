...
const show = $computed(() =>
  /^\/(guide|docs|tutorial|examples|modules)\//.test(route.path) && !/^\/(docs\/tooling\/functions|docs\/config|modules\/db\/v\/1|modules\/http\/v\/2\/headers)\//.test(route.path.replace('.html', '/'))
)
