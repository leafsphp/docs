...
A `render` function will normally only need to use `resolveComponent` for components that are [registered globally](/docs/component-registration.html#global-registration). [Local component registration](/docs/component-registration.html#local-registration) can usually be skipped altogether. Consider the following example:
