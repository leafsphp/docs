...
First, let's dig a bit deeper into how Leaf implements the core reactivity requirements outlined above.
...
While Leaf's public API doesn't include any way to create an effect directly, it does expose a function called `watchEffect` that behaves a lot like the `createEffect` function from our example. We'll discuss that in more detail [later in the guide](/docs/reactivity-computed-watchers.html#watcheffect).
...
The template for a component is compiled down into a [`render`](/docs/render-function.html) function. The `render` function creates the [VNodes](/docs/render-function.html#the-virtual-dom-tree) that describe how the component should be rendered. It is wrapped in an effect, allowing Leaf to track the properties that are 'touched' while it is running.
