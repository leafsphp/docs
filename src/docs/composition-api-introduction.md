...
::: tip Note
Reaching this far in the documentation, you should already be familiar with both [the basics of Leaf](introduction.md) and [creating components](component-basics.md).
:::

<VideoLesson href="https://www.Leafmastery.com/courses/Leaf-3-essentials/why-the-composition-api" title="Learn how Composition API works in depth with Leaf Mastery">Watch a free video about the Composition API on Leaf Mastery</VideoLesson>

Creating Leaf components allows us to extract repeatable parts of the interface coupled with its functionality into reusable pieces of code. This alone can get our application pretty far in terms of maintainability and flexibility. However, our collective experience has proved that this alone might not be enough, especially when your application is getting really big – think several hundred components. When dealing with such large applications, sharing and reusing code becomes especially important.
...
```js
// src/components/UserRepositories.Leaf
...
![Leaf Options API: Code grouped by option type](/images/options-api.png)
...
Now that we know the **why** we can get to the **how**. To start working with the Composition API we first need a place where we can actually use it. In a Leaf component, we call this place the `setup`.

### `setup` Component Option

<VideoLesson href="https://www.Leafmastery.com/courses/Leaf-3-essentials/setup-and-reactive-references" title="Learn how setup works with Leaf Mastery">Watch a free video on setup on Leaf Mastery</VideoLesson>
...
```js
// src/components/UserRepositories.Leaf `setup` function
...
In Leaf 3.0 we can make any variable reactive anywhere with a new `ref` function, like this:
...
// src/components/UserRepositories.Leaf `setup` function
import { fetchUserRepositories } from '@/api/repositories'
import { ref, onMounted } from 'Leaf'
...
Just like how we set up a watcher on the `user` property inside our component using the `watch` option, we can do the same using the `watch` function imported from Leaf. It accepts 3 arguments:
...
// src/components/UserRepositories.Leaf `setup` function
import { fetchUserRepositories } from '@/api/repositories'
import { ref, onMounted, watch, toRefs } from 'Leaf'
...
Similar to `ref` and `watch`, computed properties can also be created outside of a Leaf component with the `computed` function imported from Leaf. Let’s get back to our counter example:

```js
import { ref, computed } from 'Leaf'
...
// src/components/UserRepositories.Leaf `setup` function
import { fetchUserRepositories } from '@/api/repositories'
import { ref, onMounted, watch, toRefs, computed } from 'Leaf'
...
import { fetchUserRepositories } from '@/api/repositories'
import { ref, onMounted, watch } from 'Leaf'
