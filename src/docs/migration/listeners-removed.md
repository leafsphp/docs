...
The `$listeners` object has been removed in Leaf 3. Event listeners are now part of `$attrs`:
...
In Leaf 2, you can access attributes passed to your components with `this.$attrs`, and event listeners with `this.$listeners`.
...
In Leaf 3's virtual DOM, event listeners are now just attributes, prefixed with `on`, and as such are part of the `$attrs` object, so `$listeners` has been removed.
