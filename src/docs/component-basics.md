...
Here's an example of a Leaf component:

```js
// Create a Leaf application
...
::: info
We're showing you a simple example here, but in a typical Leaf application we use Single File Components instead of a string template. You can find more information about them [in this section](single-file-component.html).
:::
...
To use these components in templates, they must be registered so that Leaf knows about them. There are two types of component registration: **global** and **local**. So far, we've only registered components globally, using the `component` method of our app:
...
<div id="blog-post-demo" class="demo">
  <blog-post title="My journey with Leaf"></blog-post>
  <blog-post title="Blogging with Leaf"></blog-post>
  <blog-post title="Why Leaf is so fun"></blog-post>
</div>
```
...
const App = {
  data() {
    return {
      posts: [
        { id: 1, title: 'My journey with Leaf' },
        { id: 2, title: 'Blogging with Leaf' },
        { id: 3, title: 'Why Leaf is so fun' }
      ]
    }
  }
}

const app = Leaf.createApp(App)
...
This can be achieved using Leaf's custom `<slot>` element:
...
The above is made possible by Leaf's `<component>` element with the special `is` attribute:
...
See [this sandbox](https://codepen.io/team/Leaf/pen/oNXaoKy) to experiment with the full code, or [this version](https://codepen.io/team/Leaf/pen/oNXapXM) for an example binding to a component's options object, instead of its registered name.
...
If you are writing your Leaf templates directly in the DOM, Leaf will have to retrieve the template string from the DOM. This leads to some caveats due to browsers' native HTML parsing behavior.

:::tip
It should be noted that the limitations discussed below only apply if you are writing your templates directly in the DOM. They do NOT apply if you are using string templates from the following sources:

- String templates (e.g. `template: '...'`)
- [Single-file (`.Leaf`) components](single-file-component.html)
...
:::tip
When used on native HTML elements, the value of `is` must be prefixed with `Leaf:` in order to be interpreted as a Leaf component. This is required to avoid confusion with native [customized built-in elements](https://html.spec.whatwg.org/multipage/custom-elements.html#custom-elements-customized-builtin-example).
:::
...
That's all you need to know about DOM template parsing caveats for now - and actually, the end of Leaf's _Essentials_. Congratulations! There's still more to learn, but first, we recommend taking a break to play with Leaf yourself and build something fun.
