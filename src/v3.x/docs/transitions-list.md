...
- [Transition modes](/v3.x/docs/transitions-enterleave.html#transition-modes) are not available, because we are no longer alternating between mutually exclusive elements.
...
Leaf.createApp(Demo).mount('#list-demo')
```
...
Leaf.createApp(Demo).mount('#flip-list-demo')
```
...
This might seem like magic, but under the hood, Leaf is using an animation technique called [FLIP](https://aerotwist.com/blog/flip-your-animations/) to smoothly transition elements from their old position to their new position using transforms.
...
Leaf.createApp(Demo).mount('#list-complete-demo')
```
...
These FLIP animations are also not limited to a single axis. Items in a multidimensional grid can be [transitioned too](https://codesandbox.io/s/github/leafphp/leafphp.org/tree/master/src/v2/examples/Leaf-20-list-move-transitions):
...
Transitions can be reused through Leaf's component system. To create a reusable transition, all you have to do is place a `<transition>` or `<transition-group>` component at the root, then pass any children into the transition component.

TODO: refactor to Leaf 3

Here's an example using a template component:

```js
Leaf.component('my-special-transition', {
  template: '\
    <transition\
...
Yes, even transitions in Leaf are data-driven! The most basic example of a dynamic transition binds the `name` attribute to a dynamic property.
...
This can be useful when you've defined CSS transitions/animations using Leaf's transition class conventions and want to switch between them.
