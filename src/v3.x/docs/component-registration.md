...
The name you give a component may depend on where you intend to use it. When using a component directly in the DOM (as opposed to in a string template or [single-file component](../v3.x/docs/single-file-component.html)), we strongly recommend following the [W3C rules](https://html.spec.whatwg.org/multipage/custom-elements.html#valid-custom-element-name) for custom tag names:
...
```js
Leaf.createApp({...}).component('my-component-name', {
  // ... options ...
})
```
...
```js
import ComponentA from './ComponentA.Leaf'
...
Then you'll need to import each component you'd like to use, before you locally register it. For example, in a hypothetical `ComponentB.js` or `ComponentB.Leaf` file:
