...
Leaf recommends using templates to build applications in the vast majority of cases. However, there are situations where we need the full programmatic power of JavaScript. That's where we can use the **render function**.
...
Updating all these nodes efficiently can be difficult, but thankfully, we never have to do it manually. Instead, we tell Leaf what HTML we want on the page, in a template:
...
And in both cases, Leaf automatically keeps the page updated, even when `blogTitle` changes.

## The Virtual DOM tree

Leaf keeps the page updated by building a **virtual DOM** to keep track of the changes it needs to make to the real DOM. Taking a closer look at this line:

```js
return h('h1', {}, this.blogTitle)
```

What is the `h()` function returning? It's not _exactly_ a real DOM element. It returns a plain object which contains information describing to Leaf what kind of node it should render on the page, including descriptions of any child nodes. We call this node description a "virtual node", usually abbreviated to **VNode**. "Virtual DOM" is what we call the entire tree of VNodes, built by a tree of Leaf components.
...
A `render` function will normally only need to use `resolveComponent` for components that are [registered globally](/v3.x/docs/component-registration.html#global-registration). [Local component registration](/v3.x/docs/component-registration.html#local-registration) can usually be skipped altogether. Consider the following example:
...
Wherever something can be easily accomplished in plain JavaScript, Leaf render functions do not provide a proprietary alternative. For example, in a template using `v-if` and `v-for`:
...
```js
const { h, resolveDynamicComponent } = Leaf
...
```js
const { h, resolveDirective, withDirectives } = Leaf
...
```js
const { h, KeepAlive, Teleport, Transition, TransitionGroup } = Leaf
...
To create a functional component we use a plain function, rather than an options object. The function is effectively the `render` function for the component. As there is no `this` reference for a functional component, Leaf will pass in the `props` as the first argument:
...
You may be interested to know that Leaf's templates actually compile to render functions. This is an implementation detail you usually don't need to know about, but if you'd like to see how specific template features are compiled, you may find it interesting. Below is a little demo using `Leaf.compile` to live-compile a template string:

<iframe src="https://Leaf-next-template-explorer.netlify.app/" width="100%" height="420"></iframe>
