...
This section aims to provide guidance to navigating the testing ecosystem and choosing the right tools for your Leaf application or component library.
...
Unit testing a Leaf application does not significantly differ from testing other types of applications.
...
While there are many tools in the ecosystem, here are some common unit testing tools that are being used in the Leaf PHP ecosystem.
...
- [Official Leaf 2 CLI Plugin - Jest](https://cli.leafphp.org/core-plugins/unit-jest.html)
...
- [Official Leaf CLI Plugin - Mocha](https://cli.leafphp.org/core-plugins/unit-mocha.html)

## Component Testing

### Introduction

To test most Leaf components, they must be mounted to the DOM (either virtual or real) in order to fully assert that they are working. This is another framework-agnostic concept. As a result, component testing frameworks were created to give users the ability to do this reliably while also providing Leaf-specific conveniences such as integrations for Leafx, Leaf Router, and other Leaf plugins.
...
#### Optimal compatibility with the Leaf ecosystem

It should be no surprise that one of the first criteria is that a component testing library should have is being as compatible with the Leaf ecosystem as possible. While this may seem comprehensive, some key integration areas to keep in mind include single file components (SFCs), Leafx, Leaf Router, and any other Leaf specific plugins that your application relies on.
...
#### Leaf Testing Library (@testing-library/Leaf)

Leaf Testing Library is a set of tools focused on testing components without relying on implementation details. Built with accessibility in mind, its approach also makes refactoring a breeze.

Its guiding principle is that the more tests resemble the way software is used, the more confidence they can provide.

**Resources:**

- [Official Leaf Testing Library Website](https://testing-library.com/v3.x/docs/Leaf-testing-library/intro)

#### Leaf Test Utils

Leaf Test Utils is the official low-level component testing library that was written to provide users access to Leaf specific APIs. If you are new to testing Leaf applications, we would recommend using Leaf Testing Library, which is an abstraction over Leaf Test Utils.

**Resources:**

- [Official Leaf Test Utils Documentation](https://Leaf-test-utils.leafphp.org)
- [Leaf Testing Handbook](https://lmiller1990.github.io/Leaf-testing-handbook/v3/#what-is-this-guide) by Lachlan Miller
...
While there are many tools in the ecosystem, here are some common end-to-end (E2E) testing frameworks that are being used in the Leaf PHP ecosystem.
...
- [Official Leaf CLI Cypress Plugin](https://cli.leafphp.org/core-plugins/e2e-cypress.html)
...
- [Official Leaf CLI Nightwatch Plugin](https://cli.leafphp.org/core-plugins/e2e-nightwatch.html)
