...
We have created a standalone guide for creating server-rendered Leaf applications. This is a very in-depth guide for those who are already familiar with client-side Leaf development, server-side Node.js development and webpack. Check it out [here](/v3.x/docs/ssr/introduction.html).
...
Vite also provides built-in [support for server-side rendering](https://vitejs.dev/v3.x/docs/ssr.html). You can find an example project with Leaf [here](https://github.com/vitejs/vite/tree/main/packages/playground/ssr-Leaf)
