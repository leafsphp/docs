...
<script>
  ...
  export default {
    ...
    setup(props) {
      ...
      const book = reactive({ title: 'Leaf 3 Guide' })
...
Note that [refs](../api/refs-api.html#ref) returned from `setup` are [automatically shallow unwrapped](/v3.x/docs/reactivity-fundamentals.html#ref-unwrapping) when accessed in the template so you shouldn't use `.value` in templates.
...
import { h, ref, reactive } from 'Leaf'
